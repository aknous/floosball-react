...
| `/games` | `Views/GameBoard/GameBoardPage` | Every game of the week, Large (2-up) or Small (4-up), fixed interest ranking, active-rules strip. **Prognostication picks live on the win-probability gauge** — the abbr+% labels either side of the split bar ARE the pick buttons (`GameBoard/pickControl.tsx`, shared by both densities), which is where the old dashboard put them and the only spot already flanking the bar. ⚠️ **CURRENT WEEK ONLY**: `usePickEm` returns this week's fixtures and the board scrolls back to past ones, so a past-week view is handed `null` per card rather than this week's picks against last week's games. ⚠️ **KEYED BY THE TEAM PAIR, NEVER BY LIST POSITION** — the same rule `_liveGameFor` enforces server-side after index-matching put 11 of 16 pick-em cards against the wrong game on production. ⚠️ **A LIVE OR FINAL GAME IS NEVER PICKABLE, gated on `game.status` HERE rather than on the fetched `pickable` flag.** The API already closes picks at kickoff (`_pickemPickable` rejects Active and Final) and `usePickEm` refetches on `game_start`, but that leaves a window between the kickoff and the refetch landing — or a missed socket event — where the cached flag still says open and the reader gets a button that submits into a rejection. The board holds live status continuously, so it is the authority. ⚠️ The control's padding and border exist in every state (transparent when unset) so the gauge cannot shift a pixel as a pick lands, and it `stopPropagation`s so picking does not also open the game modal. With no pick state it renders the plain label exactly as before, so signed-out readers are unchanged. |
| `/standings` | `Views/Standings/StandingsPage` | Three views: By division / By league / Wild card race |
| `/prognostications` | `Views/Prognostications/PrognosticationsPage` | Head-to-head `MatchupCard`s + rail (your season, auto-pick, leaderboard). Picks save themselves — the bottom bar is an acknowledgement, and it only sits there when a save FAILED or the game had already kicked off. Settled games show a green check + the points, a red X, or a lock while a game is underway | ⚠️ Cards are `minmax(**480px**, 1fr)`, MEASURED not guessed: a Side is a 34px crest, the city/name block and a 19px win% sharing one row with space-between, so at the old 400px the name got **62px against the 97px "Grillmeisters" needs** and most club names truncated. 480 is the first width where the longest name in the league fits whole.
...
- ⚠️ **A CONDITIONAL NAV ENTRY MUST READ THE BACKEND'S OWN FLAG, NOT RE-DERIVE IT.** `/api/season` returns **`bracket_available`** — computed from whether the playoff seeds are FROZEN, with a comment saying it exists to drive the Bracket nav item — and `Navbar.js` / `Sidebar.tsx` both read it. `AppNav` re-derived the answer as `currentWeek > 28 || seasonComplete`, which is a **different and later** question: seeds freeze when the field is set, week 29 is when the first game kicks off. Between those two moments the bracket is open for entries and the only link to it was missing, so **the challenge was unreachable during the window it exists for** — reported as the Bracket tab not appearing. Now `bracketAvailable || seasonComplete` (the second keeps the entry through the gap after the Floos Bowl, where frozen seeds are cleared), and `REGULAR_SEASON_WEEKS` is gone from the nav entirely. ⚠️ It also carries a **notification dot until a ballot is in** — the nav's rule is that a dot goes only on a tab that genuinely notifies, and an unfilled bracket is a queue you can empty. `GET /api/playoffs/bracket/me` returns `hasBracket`; the check is gated on `bracketAvailable && user` so it costs nothing for the ~28 weeks it does not apply, defaults to "in" so a dot never flashes on load or after a failed check, and is cleared by a **`floosball:bracket-submitted`** window event that `usePlayoffBracket.submit` dispatches — the submit already knows, so the nav never polls. The mark is the ORIGINAL stroked tournament tree from `Sidebar` (owner): the filled version that replaced it read as four corner ticks, because a bracket is lines MERGING and there is no tree left once you fill it — hence `LINE_ICON` alongside `ICON`. ⚠️ **BRACKET COLUMNS HAVE A FLOOR** (`BRACKET_COL_MIN` 184): they were `flex: '1 1 0'` with `minWidth: 0` on desktop, to "fit without horizontal scroll" — but a flex item with no floor shrinks to whatever is left, so names truncated and the container's own `overflowX: auto` could never fire. MEASURED at 700 13px: row chrome (seed 14 + crest 24 + score 16 + gaps + padding + border) is 100px and the longest club name, "Grillmeisters", is 82.9px. Seven columns plus gaps is **~1389px**, more than the content column offers on a 1440 screen once the sidebar is out — so it now scrolls sideways rather than truncating, which is the trade the old value had backwards. The page's `maxWidth: 1400` came off at the same time, since it was capping space the reader could see going unused. Regression: `Shell/__tests__/navGates.test.ts`.
- ⚠️ **THE SEASON-END PAGES HAD NO DOOR.** `SeasonRecap` and the Draft Board (`OffseasonPanel`) were rendered ONLY inside `DashboardNew`, which the front-page restructure left routed at `/dashboard/legacy` — `/dashboard` redirects to `/` and `FrontPage` had no offseason branch at all, so the two pages that ARE the season's ending were reachable only by someone who knew the legacy URL. They now live at **`/offseason`**, with the nav entry appearing when its moment arrives and not before — the same treatment as the Bracket and Awards, because a draft board in week 3 is an empty list. ⚠️ The offseason entry **replaces** the Bracket rather than joining it: once the drafts run the bracket is a settled result, and two postseason entries push the standing pages down. They stay ONE page with a toggle rather than two nav entries because the offseason is SEQUENTIAL — the recap is the story the moment the Floos Bowl ends, the draft board while picks are running — so two permanent entries would advertise a dead one half the time.
- ⚠️ **THE FRONT PAGE HAS AN OFFSEASON STATE, because two of its modules are game-shaped.** `LiveTicker` (the hero rail) renders the week's slate and `QuickPicks` (the personal rail) renders fixtures to pick — in the offseason there are neither, so the most prominent column on the landing page and the bottom of the rail were both dead for the whole break, at the moment the league is at its most active. They swap to `OffseasonHero` (the announcement + links into `/offseason`) and `SeasonOverCard` (champion / MVP / HoF count, plus the reader's own finish in the fantasy, pick-em and bracket boards). Gate is `seasonState?.currentWeekText === 'Offseason'` — the same string the legacy dashboard used, so the page and the nav cannot disagree. ⚠️ `useSeasonRecap(enabled)` is GATED on it: `/api/recap` is a consolidated payload (awards, standings, leaders, every transaction, four fan leaderboards) and nothing on the front page reads it out of season, so an ungated hook would pull the whole thing on every landing-page load for eleven months of the year. ⚠️ There is deliberately NO "draft is live" indicator — the front page's `gamesActive` is `games.some(status === 'Active')`, which is false all offseason by definition, so wiring it would ship a light that can never come on. A real one needs the sim's offseason phase, which the frontend is not told (`_offseasonFlowPhase` is backend-only). ⚠️ **`OffseasonHero` IS A FULL-WIDTH BAND, NOT A RAIL** — `.frontGrid > .frontHero` is `grid-column: 1 / -1`, which is right for `LiveTicker` (a horizontal slate) and was never true of the hero's own layout. It was built as a narrow-column card (`display: block` CTAs stacked in a `flexDirection: column`), and its own docstring called the slot "the most prominent COLUMN" — so at the real width its two links rendered as page-spanning bars. Reported 2026-08-14 as the offseason rail being "way too wide". The CTAs now size to their labels and sit side by side; `flex: 0 0 auto` on each AND `width: fit-content` on the row are both required, since either alone still lets the grid stretch them. `flexWrap` keeps it right on a phone, where the grid collapses to one column and the hero genuinely is narrow. ⚠️ The lesson generalises: this file and that docstring both described the slot as a column, so the component was written against the PROSE rather than the CSS — check `index.css` for the actual `grid-column` before laying anything out in `.frontHero`.
