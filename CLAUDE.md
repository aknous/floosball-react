...
- ⚠️ **A CONDITIONAL NAV ENTRY MUST READ THE BACKEND'S OWN FLAG, NOT RE-DERIVE IT.** `/api/season` returns **`bracket_available`** — computed from whether the playoff seeds are FROZEN, with a comment saying it exists to drive the Bracket nav item — and `Navbar.js` / `Sidebar.tsx` both read it. `AppNav` re-derived the answer as `currentWeek > 28 || seasonComplete`, which is a **different and later** question: seeds freeze when the field is set, week 29 is when the first game kicks off. Between those two moments the bracket is open for entries and the only link to it was missing, so **the challenge was unreachable during the window it exists for** — reported as the Bracket tab not appearing. Now `bracketAvailable || seasonComplete` (the second keeps the entry through the gap after the Floos Bowl, where frozen seeds are cleared), and `REGULAR_SEASON_WEEKS` is gone from the nav entirely. Regression: `Shell/__tests__/navGates.test.ts`.
