...
const EDITION_MINI: Record<string, {
  ...
  base: { border: '#414a5c', bg: 'linear-gradient(135deg, #2b3242, #232a36, #2a313f)', label: 'Base' },
  metallic: { border: '#8a9bb4', bg: 'linear-gradient(135deg, #38414f, #313945, #434c5b)', label: 'Metallic', glow: 'rgba(154,171,196,0.18)' },
      ...
      {!expanded && !loading && (
        <div style={{
          ...
          {Array.from({ length: numSlots }, (_, i) => i + 1).map(slotNum => {
            ...
            const edition = slot.card.edition || 'metallic'
            const ed = EDITION_MINI[edition] ?? EDITION_MINI.metallic
